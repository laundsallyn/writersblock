from flaskapp.scraper.goodreads.models.series import GoodreadsSeries
from flaskapp.scraper.goodreads.models.book import GoodreadsBook
...
class GoodreadsAuthor(object):
    ...
    def series(self):
        ...
        if type(self._author_dict['books']['book']) == list:
            return [GoodreadsSeries(book_dict, self._client)
                    for book_dict in self._author_dict['books']['book']]
        else:
            return \
                [GoodreadsBook(self._author_dict['books']['book'], self._client)]
