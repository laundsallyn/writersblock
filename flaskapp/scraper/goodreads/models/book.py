...
from flaskapp.scraper.goodreads.models import series


class GoodreadsBook(object):
    def __init__(self, book_dict: dict, client):
    ...
    def work_id(self):
        ...
        return self._book_dict['work']['id']['#text']

    @property
    def series_work__id(self):
        """Return series of the book"""
        if self._book_dict['series_works'] \
                and self._book_dict['series_works']['series_work']:
            if type(self._book_dict['series_works']['series_work']) == list:
                return self._book_dict['series_works']['series_work'][0]
            else:
                return self._book_dict['series_works']['series_work']['id']
        else:
    ...
    def series_id(self):
        """Return series of the book"""
        if self._book_dict['series_works']['series_work']['series']:
            return self._book_dict['series_works']['series_work']['series']['id']
        else:
            print(self._book_dict['series_works']['series_work'])
            return None
