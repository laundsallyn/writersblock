...
class MyTest(TestCase):
    ...
    def test_author_query_1(self):
        book = self.session.query(Author).filter_by(name = 'Fyodor Dostoyevsky').first()
    ...
    def test_add_book_3 (self):
        ...
        test = self.session.query(Author).filter_by(name = written_by).first()
        book = test.wrotes[0]
        self.assertEqual(book.title,"Colorless tsukuru Tazaki and His Years of Pilgrimage")
        self.assertEqual(book.written_by[0].name, "Haruki Murakami")

    def test_wrotes_1 (self):
        author = self.session.query(Author).filter_by(name = 'Fyodor Dostoyevsky').first()
        self.assertEqual(len(author.wrotes.all()), 2) 

    def test_book_query_2 (self):
        book = self.session.query(Book).filter_by(title = 'Notes from Underground').first()
    ...
    def test_book_query_3 (self):
        book = self.session.query(Book).filter_by(title = 'Crime and Punishment').first()
    ...
    def test_lord_of_the_rings_1(self):
        book = self.session.query(Series).filter_by(title = 'The Lord of the Rings').first()
        self.assertEqual(book.written_by[0].name, "J. R. R. tolkien") 
        self.assertEqual(book.num_books, 3)
        self.assertEqual(book.status, 1)
   
    def test_num_books_in_rings (self):
        series = self.session.query(Series).filter_by(title = 'The Lord of the Rings').first()
